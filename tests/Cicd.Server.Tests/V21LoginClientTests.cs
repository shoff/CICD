...
public class V21LoginClientTests
{
    ...
    /// <summary>Records the request (body included) and returns a canned response.</summary>
    ...
    public async Task Posts_pascal_case_credentials_to_the_derived_login_url()
    {
        ...
        var identity = await Client(handler).LoginAsync("asmith", "s3cret", CancellationToken.None);
    ...
    {
        var handler = TokenHandler(new { sub = "42" });
        var identity = await Client(handler, o => o.Authority = "https://idp.example/").LoginAsync("asmith", "s3cret", CancellationToken.None);
    ...
    public async Task Username_claim_wins_over_unique_name()
    {
        ...
        var identity = await Client(handler).LoginAsync("alice", "s3cret", CancellationToken.None);
    ...
    public async Task Rejected_credentials_yield_null(HttpStatusCode status)
    {
        var handler = new FakeHandler(status, """{"error":"invalid_grant"}""");
        Assert.Null(await Client(handler).LoginAsync("asmith", "wrong", CancellationToken.None));
    }

    [Fact]
    public async Task Success_without_a_token_yields_null()
    {
        var handler = new FakeHandler(HttpStatusCode.OK, """{"refresh_token":"r"}""");
        Assert.Null(await Client(handler).LoginAsync("asmith", "s3cret", CancellationToken.None));
    }

    [Fact]
    public async Task Token_without_a_subject_yields_null()
    {
        var handler = TokenHandler(new { name = "Alice Smith" });
        Assert.Null(await Client(handler).LoginAsync("asmith", "s3cret", CancellationToken.None));
    }
    ...
    {
        var handler = TokenHandler(new { sub = "42" });
        var identity = await Client(handler, o => o.LoginUrl = "https://idp.example/custom/v21/login")
