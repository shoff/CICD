...
/// <summary>
/// One managed configuration key. Key is the configuration path; Section is its first segment.
/// DefaultValue is what the seeder stores when configuration has nothing for the key, so booleans and numbers
/// get a real value instead of an empty string the binder cannot read.
/// </summary>
...
public static class SettingsCatalog
{
    ...
    [
        ...
        new("Server:DispatchIntervalSeconds", "Server", "Dispatch interval (s)", "How often queued builds are assigned to agents.", SettingKind.Number, DefaultValue: "2"),
        new("Server:TriggerPollIntervalSeconds", "Server", "Trigger poll interval (s)", "How often VCS and other triggers are polled.", SettingKind.Number, DefaultValue: "30"),
        new("Server:PullRequestPollIntervalSeconds", "Server", "Pull request poll interval (s)", "How often open pull requests are refreshed.", SettingKind.Number, DefaultValue: "60"),
        new("Agents:AuthToken", "Agents", "Agent auth token", "Shared secret every agent presents when connecting.", SettingKind.Secret),
        new("Agents:AutoAuthorize", "Agents", "Auto-authorize agents", "Authorize new agents on first registration. Development only.", SettingKind.Boolean, DefaultValue: "false"),
        ...
        new("IdentityProvider:ReturnUrl", "IdentityProvider", "Return URL", "Sent to the login endpoint; it requires a value.", SettingKind.Text, DefaultValue: "http://localhost:7003/callback.html"),
        new("IdentityProvider:ValidateAudience", "IdentityProvider", "Validate audience", "Validate the audience of API bearer tokens.", SettingKind.Boolean, RestartRequired: true, DefaultValue: "false"),
        ...
        new("IdentityProvider:RequireHttpsMetadata", "IdentityProvider", "Require HTTPS", "Refuse plain-http provider URLs.", SettingKind.Boolean, RestartRequired: true, DefaultValue: "true"),
        new("IdentityProvider:TimeoutSeconds", "IdentityProvider", "Login timeout (s)", "Timeout for the sign-in call.", SettingKind.Number, DefaultValue: "15"),
        ...
        new("GitHub:ApiBaseUrl", "GitHub", "API base URL", "GitHub REST API base.", SettingKind.Text, DefaultValue: "https://api.github.com/"),
