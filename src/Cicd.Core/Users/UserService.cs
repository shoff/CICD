...
public sealed class UserService(CicdDbContext db, IOptionsMonitor<SecurityOptions> security, TimeProvider clock)
{
    ...
    private bool IsBootstrapAdmin(string? email) =>
        email is not null && security.CurrentValue.BootstrapAdmins.Any(a => string.Equals(a, email, StringComparison.OrdinalIgnoreCase));
}
